...
vcovA <- function(CoQR_object, sparsity="kernel", bandwidth="MachadoSilva",...) {
  ...
  # Check if that is correct in Yannick's new version
  ...
  # Check if it works with the dimensions!!!
...
sparsity_kernel <- function(x, y, v, c, beta, alpha, TT, bandwidth){
  if (bandwidth=="MachadoSilva"){
    ...
    # OLD c_T implementation!
    # c_T <- mad(y-c) * (TT)^(-1/3) * (qnorm( min(1-eps, (1-beta)*(1-alpha)+m_T((1-alpha)*(1-beta)))) - qnorm( max(eps,(1-beta)*(1-alpha)-m_T((1-alpha)*(1-beta)))))
  ...
  # mean(fY_est > 0)
  # xmv <- x-v
  # hist(xmv[abs(xmv)<b_T])
  # ymc <- y-c
  # hist(ymc[abs(ymc)<c_T])

  # ind_1 <- which(abs(xmv)<b_T & xmv < 0)
...
sparsity_ARB <- function(x, y, v, c, beta, alpha, TT, nabla_v, Xi_vv_vec){
  # Adaptiv Random Bandwidth (ARB) method of Hecq and Sun (2022)
  # So far, this only works for f_X(v), but not for the other partial derivatives
  # ToDo: Generalize for the other derivatives of the CDF
  # Use the standard kernel estimators so far.
...
vcovB <- function(CoQR_object, block_length='default', B=500) {
  if (B < 500)
    warning("The number of bootstrap iterations is small!")

  data <- dplyr::select(CoQR_object$data, -c("VaR", "CoVaR"))
  ...
  theta0_list <- theta_fun(model=CoQR_object$model, theta=CoQR_object$theta, df=data)
  ...
  for (b in 1:B) {
    ...
    opt_v <- optim(par=theta01,
                   ...
                   df=data, model=CoQR_object$model, prob_level=CoQR_object$prob_level)

    theta_est_v <- opt_v$par
    m1_est <- model_fun(theta_est_v, df=data, prob_level=CoQR_object$prob_level, model=CoQR_object$model, model_type="first")$m1
    ...
    opt_m2 <- optim(par=theta02,
                    fn=function(theta,...){
                      switch(CoQR_object$SRM,
                             MES = {mean(loss_model_MES(theta,...) %>% .[B_idx],na.rm=TRUE)},
                             CoVaR = {mean(loss_model_CoVaR(theta,...) %>% .[B_idx],na.rm=TRUE)})},
                    df=data, m1=m1_est, model=CoQR_object$model, prob_level=CoQR_object$prob_level)
...
vcovBfast <- function(CoQR_object, block_length='default', B=100) {
  if (B < 500)
    warning("The number of bootstrap iterations is small!")
  ...
  # # Split the starting value (the original parameter estimate)
  theta0_list <- theta_fun(model=CoQR_object$model, theta=CoQR_object$theta, df=CoQR_object$data)
  # theta01 <- theta0_list$theta1
  # theta02 <- theta0_list$theta2

  TT <- dim(CoQR_object$data)[1]
  theta_info <- theta_fun(model=CoQR_object$model, theta=CoQR_object$theta)
  q1 <- theta_info$length_theta1
  q2 <- theta_info$length_theta2
  y <- CoQR_object$data$y
  x <- CoQR_object$data$x
  ...
  momcond <- momcond_CoVaR(CoQR_object$theta, CoQR_object$data, A, CoQR_object$model, CoQR_object$prob_level)
